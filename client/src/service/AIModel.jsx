// Enhanced AIModel.js - Connected securely to Backend AI API
import api from './api';
...
// --- 6. AI-POWERED SMART ITINERARY PLANNER (Connected to Backend AI API) ---
async function createSmartItinerary(places, hotels, days, location, traveler, budget) {
  try {
    console.log("🤖 Requesting itinerary from Backend AI Service (/api/ai/generate-itinerary)...");
    ...
    if (!aiItinerary || !Array.isArray(aiItinerary) || aiItinerary.length === 0) {
      throw new Error("Invalid itinerary structure received from AI backend");
    }

    // Map AI suggestions back to full place objects and enrich with imagery
    return aiItinerary.map(dayPlan => ({
      day: dayPlan.day,
      theme: dayPlan.theme || `Day ${dayPlan.day} Exploration`,
      area: dayPlan.area || "Downtown",
      suggestedHotel: dayPlan.suggestedHotel || hotels[0]?.hotelName || "Recommended Hotel",
      totalTime: dayPlan.totalTime || "8h",
      totalExpense: dayPlan.totalExpense || "Moderate",
      places: (dayPlan.places || []).map((p, idx) => {
        const original = places.find(orig => orig.placeName?.toLowerCase() === p.placeName?.toLowerCase());
        const finalName = p.placeName || `${location} Landmark ${idx + 1}`;

        return {
          placeName: finalName,
          details: original?.details || p.details || `Discover the iconic culture and history of ${finalName} in ${location}.`,
          ...
          rating: original?.rating || (4.2 + Math.random() * 0.7).toFixed(1),
          travelTime: original?.travelTime || "20-30 min",
          bestTimeToVisit: original?.bestTimeToVisit || "Morning",
          visitTime: p.visitTime || "10:00 AM",
          duration: p.duration || "2h",
          estimatedCost: p.estimatedCost || "₹500",
          nearbyTip: p.nearbyTip || "Check local opening hours before visiting."
        };
      })
    }));
  } catch (aiError) {
    console.warn("Backend AI planning failed, falling back to smart grouping:", aiError.message);
    // Graceful fallback to guarantee UI always renders properly
