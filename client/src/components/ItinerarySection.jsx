...
import { Clock, ChevronRight, ChevronDown, DollarSign, Compass, Navigation, CloudSun } from 'lucide-react';

const ItinerarySection = ({ itinerary, tripHotelName, onCheckWeather }) => {
      ...
      <div className="relative pl-6 md:pl-8 pb-4 group/place">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 py-1 px-2 transition-all">
          ...
          <div className="flex items-center gap-4 shrink-0">
            ...
            {onCheckWeather && (
              <button
                onClick={() => onCheckWeather(place.placeName)}
                title="Check place weather"
                className="group/weather flex items-center gap-1.5 px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 rounded-full transition-all duration-300 active:scale-95"
              >
                <CloudSun className="w-3 h-3 text-amber-400 group-hover/weather:scale-110 transition-transform" />
                <span className="text-[10px] font-inter font-black uppercase tracking-[0.2em] text-gray-400 group-hover/weather:text-white">
                  Weather
                </span>
              </button>
            )}
