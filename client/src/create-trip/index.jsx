    ...
    try {
      ...
      if (err.response?.status === 401) {
        toast({
          ...
          description: "Your login session has expired. Please log in again to generate your trip.",
          variant: "destructive"
        });
        navigate('/login');
      } else {
        const errorMsg = err.response?.data?.message || err.message || "Failed to generate trip. Please try again.";
        toast({ title: "Generation Error", description: errorMsg, variant: "destructive" });
      }
    } finally {
      setLoading(false);
    }
