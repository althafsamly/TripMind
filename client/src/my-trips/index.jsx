...
import { Calendar, Trash2, MapPin, Check, X } from 'lucide-react'
...
function MyTrips() {
    ...
    const handleRemoveTrip = async (id) => {
        if (!window.confirm("Are you sure you want to delete this trip? This will also remove all bookings and join requests.")) return;
        try {
    ...
    const handleUpdateBookingStatus = async (requestId, newStatus) => {
        try {
            await api.put(`/bookings/${requestId}/status`, { status: newStatus });
            // Update local state dynamically
            setCreatorRequests(prev => prev.map(req => 
                                            ...
                                            <div className='flex-1 flex flex-col'>
                                                ...
                                                <div className='mt-auto space-y-2 pt-3 border-t border-white/10'>
                                                    {booking.status === 'approved' && (
                                                        <Link
                                                            to={`/view-trip/${booking.tripId?._id || booking.tripId}`}
                                    ...
                                    <div className='relative z-10'>
                                        ...
                                        <div className='mt-5 pt-4 border-t border-white/10 flex flex-col gap-3'>
                                            <div className='flex items-center justify-between'>
                                                ...
                                                <Link to={`/view-trip/${request.tripId?._id || request.tripId}`} className='text-[12px] font-inter font-bold text-white hover:text-gray-300 flex items-center gap-1 transition-colors'>
                                                    View Trip
                                            ...
                                            <div className='flex items-center gap-2 pt-2'>
                                                {request.status !== 'approved' && (
                                                    <button
                                                        onClick={() => handleUpdateBookingStatus(request._id, 'approved')}
                                                        className='flex-1 h-8 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-400 border border-emerald-500/30 rounded-lg text-[11px] font-inter font-bold flex items-center justify-center gap-1 transition-colors cursor-pointer'
                                                ...
                                                {request.status !== 'rejected' && (
                                                    <button
                                                        onClick={() => handleUpdateBookingStatus(request._id, 'rejected')}
                                                        className='flex-1 h-8 bg-red-500/20 hover:bg-red-500/30 text-red-400 border border-red-500/30 rounded-lg text-[11px] font-inter font-bold flex items-center justify-center gap-1 transition-colors cursor-pointer'
